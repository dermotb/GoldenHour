...
internal sealed class SunTimes
{
                                            ...
                                            ref bool isSunrise, ref bool isSunset)
    {
		...
		string dstName = TimeZoneInfo.Local.DaylightName;

        lock (mLock)    // lock for thread safety
        {
//			double zone = -(int)Math.Round(TimeZoneInfo.Utc.GetUtcOffset(date).TotalSeconds / 3600);
			double zone = -TimeZoneInfo.Utc.GetUtcOffset(date).TotalSeconds / 3600;
