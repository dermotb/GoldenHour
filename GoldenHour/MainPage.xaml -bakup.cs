...
/*		private void geolocator_PositionChanged(Geolocator sender, PositionChangedEventArgs args)
		{
			try
			{
				if (usingCurrentPosition)
				{
					// Need to set map view on UI thread.
					this.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, new DispatchedHandler(
						() =>
						{
							displayPosition(args.Position.Coordinate.Latitude, args.Position.Coordinate.Longitude);
						}));

					this.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, new DispatchedHandler(() =>
					{
						accuracy.Text = args.Position.Coordinate.Accuracy.ToString();
					}));
			...
			catch (Exception e)
			{
				...
				this.Dispatcher.RunAsync(CoreDispatcherPriority.Normal, new DispatchedHandler(() =>
				{
					status.Text = "An error has occurred processing position data";
				}));
			}
